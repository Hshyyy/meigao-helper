          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-20">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" inputMode="numeric" pattern="[0-9]*" value={profile.toefl || ""} onChange={(e) => { const v = e.target.value.replace(/[^0-9.]/g, ''); handleNumberInput("toefl", Number(v) || 0, 0, 120, "托福成绩", 0, v); }} placeholder="例如：100" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required />
              ...
              <div>
                ...
                <input type="text" inputMode="numeric" pattern="[0-9]*" value={profile.ssat || ""} onChange={(e) => { const v = e.target.value.replace(/[^0-9.]/g, ''); handleNumberInput("ssat", Number(v) || 0, 0, 99, "SSAT 百分位", 0, v); }} placeholder={canSkipSSAT ? "不填则免 SSAT 学校优先" : "例如：85"} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required={!canSkipSSAT} />
