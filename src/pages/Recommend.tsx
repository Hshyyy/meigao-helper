          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-20">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" inputMode="numeric" pattern="[0-9]*" value={profile.toefl || ""} onChange={(e) => { const v = e.target.value.replace(/[^0-9.]/g, ''); if (v.endsWith('.')) { update("toefl", v); } else { handleNumberInput("toefl", Number(v) || 0, 0, 120, "托福成绩", 0, v); } }} placeholder="例如：100" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required />
              ...
              <div>
                ...
                <input type="text" inputMode="numeric" pattern="[0-9]*" value={profile.ssat || ""} onChange={(e) => { const v = e.target.value.replace(/[^0-9.]/g, ''); if (v.endsWith('.')) { update("ssat", v); } else { handleNumberInput("ssat", Number(v) || 0, 0, 99, "SSAT 百分位", 0, v); } }} placeholder={canSkipSSAT ? "不填则免 SSAT 学校优先" : "例如：85"} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required={!canSkipSSAT} />
              ...
              <div>
                ...
                <input type="text" inputMode="decimal" value={profile.gpa || ""} onChange={(e) => { const v = e.target.value.replace(/[^0-9.]/g, ''); if (v.endsWith('.')) { update("gpa", v); } else { handleNumberInput("gpa", Number(v) || 0, 0, profile.schoolType === "public" ? 100 : 4, profile.schoolType === "public" ? "成绩百分比" : "GPA", profile.schoolType === "public" ? 2 : 1, v); } }} placeholder={profile.schoolType === "public" ? "例如：88.5" : "例如：3.5"} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required />
                <p className="text-xs text-gray-400 mt-1">
