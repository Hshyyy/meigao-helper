          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-20">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" inputMode="numeric" value={profile.toefl || ""} onChange={(e) => { const v = e.target.value; if (v === "" || /^\d{0,3}$/.test(v)) { handleNumberInput("toefl", Number(v) || 0, 0, 120, "托福成绩", 0, v); } }} placeholder="例如：100" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required />
              ...
              <div>
                ...
                <input type="text" inputMode="numeric" value={profile.ssat || ""} onChange={(e) => { const v = e.target.value; if (v === "" || /^\d{0,2}$/.test(v)) { handleNumberInput("ssat", Number(v) || 0, 0, 99, "SSAT 百分位", 0, v); } }} placeholder={canSkipSSAT ? "不填则免 SSAT 学校优先" : "例如：85"} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required={!canSkipSSAT} />
              ...
              <div>
                ...
                {profile.schoolType === "public" ? (
                  <input type="text" inputMode="decimal" value={profile.gpa || ""} onChange={(e) => { const v = e.target.value; if (v === "" || /^\d{0,3}(\.\d{0,2})?$/.test(v)) { handleNumberInput("gpa", Number(v) || 0, 0, 100, "成绩百分比", 2, v); } }} placeholder="例如：88.5" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required />
                ) : (
                  <input type="text" inputMode="decimal" value={profile.gpa || ""} onChange={(e) => { const v = e.target.value; if (v === "" || /^\d{1}(\.\d{0,1})?$/.test(v)) { handleNumberInput("gpa", Number(v) || 0, 0, 4, "GPA", 1, v); } }} placeholder="例如：3.5" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required />
                )}
                <p className="text-xs text-gray-400 mt-1">
